...
The algorithm works by constructing a surjective mapping from the $n$ outcomes of a uniform distribution to the $k$ outputs of the weighted distribution. This uses a simple lookup table of size $n$, constructed using Algorithm \ref{alg:generate-lookup-tables}. If $n$ is very large we could implement a binary search on $offset$ instead. To generate an output, call $generate\_uniform$ to generate a integer in the range $[0,n)$, and use the number in the lookup table as the result. 

The new idea is that we can recover all of the entropy from this process. By splitting up $X$ into uniformly distributed chunks of size $w_i$, as illutrated in Figure \ref{fig:chunks}, we see that each chunk contains $\log_2w_i$ entropy. Since the entropy in each chunk is in a uniform distribution, we can use $combine$ to return this entropy back to the entropy store.

Lemma \ref{lem:distribution-conservation} shows us that when we account for this extra entropy then no entropy is lost.

\begin{definition}
    Let $f_{GD}: X \rightarrow Y, Z$ be the function that maps random variable $X \sim Uniform\{0,n-1\}$ to independent random variables $Y$ and $Z \sim Uniform\{0, w_i-1\}$.

\end{definition}

\begin{lemma}
    \label{lem:distribution-conservation}
    $f_{GD}$ does not lose entropy
\end{lemma}
...
\begin{lemma}
    The function $f_{GD}$ is a bijection.
\end{lemma}

\begin{proof}
Consider two mappings $x_1 \mapsto (y_1,z_1)$ and $x_2 \mapsto (y_2,z_2)$.

If $y_1=y_2$ and $z_1=z_2$ then this uniquely specifies 
