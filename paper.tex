...
We present new algorithms, called \em entropy store algorithms \em, that have lower setup costs and higher amortised entropy efficiency than classical algorithms. Table \ref{tab:entropy-store} summarises the algorithm, where $m$ is the number of bits in each integer, and $n$ is the size of the input/output distribution. $\epsilon$ is defined in Theorem \ref{thm:loss}. These characteristics are essentially optimal since $m$ and $n$ are generally constant.  Of course, this also covers other distributions such as uniform and Bernoulli distributions.
