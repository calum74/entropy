...
When applied to uniform distributions $X \sim Uniform\{n\}$ and $Y \sim Uniform \{m\}$, we can show that the result $f(X,Y)$ is also a uniform distribution, and that $f_{combine}$ has an inverse and is therefore a bijection. Because $f_{combine}$ is a bijection, it means it conserves entropy. Similar constructs can be found for example in \cite{gentle2003xx} XX. 
...
The function $f_{sample}$, in Definition \ref{def:sample} .. 
