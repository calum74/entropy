...
Algorithm \ref{alg:convert_u_u} shows how we can efficiently \em convert \em entropy from one uniform distribution to another, via an entropy store. This is just a two-step process, where the first \em multiply \em step is used to add the entropy from $U_n$ into $U_s$ before we generate a number $U_m$ from the same store. We'll ignore the potential overflow of $s$.
...
Algorithm \ref{alg:consume_b} can be used to read biassed bits and convert them to a uniformly distributed variable. This allows us to exploit biassed entropy sources. For a bit with bias $\frac{m}{n}$, we first generate a uniform variable of size $m$ or $n-m$, then call \em upsample \em to turn this into a uniform variable of size $n$. Then we \em multiply \em to combine the entropy in $U_s$ with $U_n$.
...
To illustrate Corollary \ref{cor:generate-uniform-efficiency}, if $N=2^{31}$ and $n=6$, then $\eta \ge 0.99999997$. This means that even with a modest entropy buffer, we can get very good entropy efficiency.
