...
In order to improve entropy efficiency further, various batching ideas have been put forward. Batching divides the overhead of generating an output over the size of the output. Blanc and Devroye s
...
Input and output distributions could also be part of a Markov chain, where the input or output distribution changes with time or on previous values.
...
often independent and identically distributed, but the inputs could also 
The output can also form a Markov

Other use cases, e.g. 


Knuth and Yao can be extended in multiple ways:
...
Random number generators are evaluated on
...
\begin{definition}
    Let $\epsilon(p)$ be the expected entropy loss function of Algorithm \ref{alg:generate-uniform}, for $p=\frac{n-1}{N}$.
\end{definition}
...
\begin{definition}
    Let $f_{gendist}: X \rightarrow Y, Z$ be the function that maps random variable $X \sim Uniform\{n\}$ to independent random variables $Y$ and $Z \sim Uniform\{w_i\}$.
...
Note that $f_{gendist}$ is a generalisation of XX $f_{combine}$ and $f_{sample}$ XX we didn't define $f_{sample}$.
