...
\begin{proof}
The values $U_n$, $n$, $U_s$ and $s$ have been generated by Algorithms \ref{alg:multiply}, \ref{alg:divide} and \ref{alg:downsample}. By Lemmas \ref{lem:multiply}, \ref{lem:downsample} and \ref{lem:divide}, $U_n$ and $U_s$ are uniformly distributed.
\end{proof}
...
\begin{proof}
For each iteration $i$ of Algorithm \ref{alg:generate-uniform}, let $p_i = \frac{s \mod n}{s} \le \frac{n-1}{N} = p$. On each iteration, the entropy lost equals
...
by Lemma \ref{lem:shannon-inequality}. This is because the $B$ value returned by \em downsample \em is consumed by the algorithm and not returned to the caller.
...
\begin{definition}
    Let the maximum conversion loss function of Algorithm \ref{alg:generate-uniform}, for $p=\frac{n-1}{N}$ be defined as
    \begin{equation}
...
To illustrate Corollary \ref{cor:convert-uniform-efficiency}, we can set $N=2^{31}$ and $n=6$. Then $\eta \ge xxxx$.
