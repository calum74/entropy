...
A common suggestion to reach asymptotic efficiency for random number generation is to use batching to generate multiple outputs at the same time. \cite{bacher2017,han97,devroye86,Knuth1976TheCO,lumbroso2013optimal} Batching spreads the entropy loss over the batch size. However batching schemes suffer from increased size and complexity, and are less flexible because you must plan beforehand which numbers to generate. The fundamental limit to batching is that they do not scale because they require arbitrary output precision or data structure sizes.
...
We'll use uppercase letters $X$, $Y$ and $Z$ for discrete random variables, and write $H(X)$ for the Shannon entropy of this variable. Write $X \sim Uniform\{0..n-1\}$ to mean that $X$ is distributed with a uniform distribution, or $X \sim Uniform\{n\}$ for short, because all distributions will be 0-based. $H(Uniform\{n\}) = \log_2n$. The interval $[a,b)$ means all integers in the range $a...b-1$. $\mathbb{P}$ means probability, $\mathbb{E}$ means expected value.


\subsection{Fundamental operations}

At the basis of the ES is the ability to extract entropy from a uniform distribution, and to combine entropy with a uniform distribution in an entropy-preserving way. These 

Algorithm \ref{alg:combine} combines entropy from two uniform discrete random variables into a single uniform random variable. It  divides the range $[0,nm)$ into $n$ subranges of $[0,m)$ as follows:

\[
\overbrace{
    \underbrace{
        \underbrace{a_0 ... a_{m-1}}_{X \sim Uniform\{m\}}
        \underbrace{a_m ... a_{2m-1}}_{X \sim Uniform\{m\}}
        ...
        \underbrace{a_{n(m-1)}...a_{nm-1}}_{X \sim Uniform\{m\}}} 
        }_{Y \sim Uniform\{n\}}
        ^{Z \sim Uniform\{nm\}}
\]

The function $f_{combine}$ defines this operation. Similar constructs can be found for example in \cite{gentle2003xx}. Algorithm \ref{alg:combine} is an implementation of $f_{combine}$.
...
\begin{lemma}
    The inverse of $f_{combine}$, $f^{-1}_{combine}(Z) = (\lfloor \frac{Z}{m} \rfloor, Y \mod m)$.
\end{lemma}
...
Algorithm \ref{alg:divide} is the inverse of Algorithm \ref{alg:combine}, allowing us to factorise a uniformly distributed integer into two. For this, the sizes of the output distributions must divide the size of the input distribution. Algorithm \ref{alg:divide} implements $f^{-1}_{combine}$.
...
Algorithm \ref{alg:resample} converts a uniformly distributed integer $Uniform\{n\}$ to a biassed bit $Bernoulli\{\frac{m}{n}\}$ and a uniform integer $Uniform\{m\}$ or $Uniform\{n-m\}$. It implements the following mapping, defined more formally in Definition \ref{def:sample} as $f_{sample}$:
...
\overbrace{
    \underbrace{
        \underbrace{a_0 \text{   } ... \text{   } a_{m-1}}_{Y \sim Uniform\{m\}}
        \underbrace{a_m \text{   } ... \text{   } a_{n-1}}_{Y \sim Uniform\{n-m\}}}
    }_{X \sim Bernoulli\{\frac{m}{n}\}}^{Z \sim Uniform\{n\}}
\]
...
\begin{definition}
$f_{sample}: [0,n) \rightarrow {0,1} \times \mathbb{N}_0$ is defined as $f_{sample}(Z) = (...).$
\label{def:sample}
\end{definition}
...
\begin{proof}
    XX define $f_{resample}(X) \rightarrow Y, Z$ where
...
Algorithm \ref{alg:resample} is the basis for von Neumann's rejection sampling algorithm, and Lumbrusco's fast dice roller algorithm. However these algorithms throw away some of the outputs, resulting in entropy inefficiency.
...
We would expect this process to lose entropy because in general the output distribution contains less entropy than the input distribution. The new idea is that we can recover all of the entropy from this process. By splitting up $X$ into chunks of size $w_i$, we see that each chunk contains a uniform distribution of size $w_i$:

\[
\overbrace{
    \underbrace{
        \underbrace{a_0 ... a_{w_0-1}}_{Y \sim Uniform\{w_0\}}
          \underbrace{a_{w_0} ... a_{w_0+w_1-1}}_{Y \sim Uniform\{w_1\}}
          ...
          \underbrace{
             a_{n-w_{k-1}} ... 
             a_{n-1}
         }_{Y \sim Uniform\{w_{k-1}\}}
