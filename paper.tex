...
Name the algorithms: Entropy store methods and algorithms
...
The main evaluation for the entropy store algorithms will be by entropy efficiency and entropy loss per output. In this case, we have already proved that the ES algorithms outperform classical algorithms such as Knuth-Yao and the Interval Algorithm, and calculated a bound, but it is still instructive to visualise this comparison.

Figure \ref{fig:uniform-losses} shows the calculated entropy loss for generating uniform integers, comparing entropy store (ES) methods with unbatched von Neumann (vN) and Knuth-Yao (KY) samplers. Note that these graphs show exact calculation, not simulation. We can observe that for vN and KY, the entropy loss is heavily dependent on the number being generated. KY loses up to 2 bits per output. ES is not dependent on the generated integer, but loses efficiency at high $n$ as it approaches its minimum store size $N$. We also show "typical" efficiency by simulating the store, and show that it is within the worst case bound $\epsilon$, but wobbles because it depends on the current size of the store.
As expected, using a larger entropy buffer yields greater efficiency, but at the cost of a larger pre-fetch of entropy and slower divmod operations. \cite{inteldivmodslowness}
...
Figure \ref{fig:shuffling-efficiency} shows the overall impact of entropy losses when applied to the shuffling a deck of $n$ cards using the Fisher-Yates algorithm \cite{fisheryates}. With an ES uniform generator, from Figure \ref{fig:uniform-losses} are negligible. As an example, we can calculate that shuffling a deck of 52 cards can be done with an entropy loss of XX, or an entropy efficiency of XX.
...
Figure \ref{fig:bernoulli-efficiency} shows the entropy efficiency of the Inverval Algorithm (IA) when generating low-entropy Bernoulli distributions. As the entropy of the output tends to zero, the efficiency of the IA also tends to zero because the IA fetches up to 2 bits. The entropy losses from ES follow Figure \ref{fig:uniform-losses} so are negligible on this graph.
...
This translates into output rate. (TODO: Output rate per bit).

We can also evaluate ES algorithms in terms of their memory, speed, setup costs and flexibility. ES can be written to use 2 integer divmod operations (see \ref{appendix}), which are likely to prove a bottleneck compared to say Lemire's algorithm \ref{lemire}, or table-based algorithms like the FLDR \ref{}. To be more precise, since integer multiplication and division is $O(n \log n)$ \ref{integer multiplication}, for an $m$ bit buffer the complexity of the algorithm is $O(m \log m)$, but for practical purposes it is $O(1)$ in time and space.

ES has another advantage, which is that it is able to generate perfect distributions and does not have (albeit minimal) truncation errors found in IA.
...
We have introduced a new class of algorithm to convert entropy to and from a uniform entropy store efficiently, and by caching unused entropy between outputs we can achieve arbitrarily low entropy losses.

We have shown that the tradition entropy efficiency limits of classical algorithms like Knuth-Yao and the Interval Algorithm can be bypassed using an entropy store. 

The algorithms are simple practical enough to be of use in low entropy regimes where the availability of entropy is a bottleneck. The difference with classical algorithms is particularly marked on Bernoulli outputs.
