...
When $X$ and $Y$ are uniformly distributed, then $f_{combine}(X,Y)$ is also uniformly distributed. $f_{combine}$ is reversible, meaning it has an inverse and is a bijection, and it conserves entropy. Similar constructs can be found for example in \cite{gentle2003xx} XX. 
...
\begin{algorithmic}[1]
    ...
    \Require $B$ is a Boolean value Bernoulli distributed with $p=\frac{m}
    \Require $U_{n}$, $m$ and $n$ are integers 
    \W $0 \le m \le n$
    \Ensure $U_x$ and $B$ are independent
    \Ensure $U_{n}$ is uniformly distributed over $[0,n)$
