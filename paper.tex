...
\begin{algorithm}
\caption{Turning a uniform into a Bernoulli and a uniform variable}
\label{alg:downsample}
\begin{algorithmic}[1]
    \Require $U_{n}$, $m$ and $n$ are integers 
    \Require $0 \le m \le n$
    \Require $U_{n} \sim Uniform \{n\}$
    \Ensure $U_{x} \sim Uniform\{x\}$
    \Ensure $x = m$ or $x=n-m$
...
\begin{algorithm}
\caption{Combining uniform and Bernoulli variables}
\label{alg:expand}
\begin{algorithmic}[1]
    \Require $U_{x}, x, B, n, m$ and $n$ are integers 
...
\begin{algorithmic}[1]
\Require $U_s, s, n, N$ are integers
\Require $n \le N$
\Require $fetch()$ returns $Bernoulli\{0.5\}$
...
\begin{algorithmic}[1]
\Require $U_s, s, m, n, N$ are integers
\Require $U_s \sim Uniform\{s\}$
\Require $0 \le m \le n\le N$
\Ensure $B \sim Bernoulli\{\frac{m}{n}\}$
\Ensure $U_s \sim Uniform\{s\}$
\Procedure{generate\_bernoulli}{$U_s, s, m, n, N$} 
...
\Require $U_s \sim Uniform\{s\}$
\Ensure $U_s \sim Uniform\{s\}$
\Ensure $i \sim Weighted\{w_i\}$
\Ensure $U_s$ and $W$ are independent
\Procedure{generate\_distribution}{$U_s, s, N, weights, outputs, offsets$} 
    \State $U_s, s, U_n \gets generate\_uniform(U_s, s, |outputs|, N)$
    \State $U_s, s = combine(U_s, s, U_n - offsets[i], weights[i])$
...
Extracting entropy means to transfer entropy from a random variable into our entropy store. The result is that the size of our entropy store $s$ increases, reflecting its increased entropy.

Extracting entropy from a uniform variate is trivial, and can be done with Algorithm \ref{alg:combine}.

For Bernoulli variables, we can use Algorithm \ref{alg:expand}, by first fetching an appropriate uniform variable from the entropy store.
...
When extracting entropy from uniform or Bernoulli variables, we can use simpler variants that bypasses the need for an $outputs$ table.
