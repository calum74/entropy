...
\begin{proof}
    Let $Z = mX+Y$, so $Z \in [0,nm)$. $\prob{z=Z} = \prob{z=mX+Y} = \sum_{x=0}^{n-1}\prob{z=mX+Y|X=x}\prob{X=x} = \frac{1}{m}\prob{X=x} = \frac{1}{m}\frac{1}{n} = \frac{1}{mn}$. Therefore, $Z$ is uniformly distributed over $[0,nm)$.
\end{proof}
...
\begin{proof}
    $\prob{X=x} = \prob{X = \lfloor \frac{Z}{m}\rfloor} = \frac{1}{n}$. $\prob{Y=y} = \prob{Y = Z \mod m} = \frac{1}{m}$.
\end{proof}
...
\begin{equation}
\entropy{B_i} \le -p\log_2p - (1-p)\log_2(1-p) 
\end{equation}
...
The efficiency of an entropy store of $m$ bits is 

\begin{align}
\eta_{min} = 1 - \Theta(m2^{-m})
\end{align}

\end{theorem}

\begin{proof}
This follows from \eqref{eq:generate-uniform-efficiency}, where we keep $n$ constant. The number of bits $m$ required by the ES is $\log_2s_{max}$, where $s_{max}$ is the maximum value in the store. The maximum value is $b$ times the minimum value $s_{min}$ (see Algorithm \ref{alg:generate-multiple}), therefore $m = \log_2(s_{max}) = \log_2(bs_{min})$, therefore $s_{min} = \frac{2^m}{b}$.
...
p   & = \frac{n-1}{s_{min}} \\
    & = \frac{b(n-1)}{2^m} \\
    & = 2^{-m}b(n-1) \\
    & = \Theta(2^{-m}) \label{eq:theta-p}\\
-\log_2 p 
    & = -\log_2 (2^{-m}b(n-1)) \\
    & = -\log_2 (2^{-m}) - \log_2(b(n-1)) \\
    & = m - \log_2(b(n-1)) \\
    & = \Theta(m) \label{eq:theta-logp}
\end{align}
...
An expansion of $\log(1+p)$ yields
...
\eta_{min} 
    & = \frac{H_{out}}{H_{out} + \epsilon} \\
    & = 1 - \frac{\epsilon}{H_{out} + \epsilon} \\
    & = 1 - \frac{\Theta(m2^{-m})}{\Theta(1)} \\
    & = 1 - \Theta(m2^{-m})
\end{align}
