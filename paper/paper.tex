...
\begin{algorithmic}[1]
    ...
    \Require $B \indep U_s$
    \Ensure $U_s \sim Uniform\{s\}$
\Procedure{combine\_bernoulli}{$U_s, s, m, n, B$} 
    \If{B=1}
        \State $x \gets m$
    \Else
        \State $x \gets n-m$
...
\Require $U_s \sim Uniform\{s\}$
    \Require $W \indep U_s$
\Ensure $U_s \sim Uniform\{s\}$
\Procedure{combine\_weighted}{$U_s, s, weights, outputs, offsets, W$} 
...
The entropy store can be used as part of a Markov sequence.
