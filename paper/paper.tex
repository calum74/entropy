...
Algorithm \ref{alg:generate-uniform} generates a uniform variable $N \sim \unif{n}$ using an entropy source $\textsc{fetch}$ and an entropy store $S \sim \unif{s}$. Excess entropy is returned in $S$. The algorithm calls $\textsc{generate\_multiple}$ to create a uniform variable $S \sim \unif{rn}$, then calls $\textsc{divide}$ (Algorithm \ref{alg:divide}) to split $S$ into $S \sim \unif{r}$ and $N \sim \unif{n}$. $N$ is our result, and $S$ is the new entropy store. A C implementation of Algorithm \ref{alg:generate-uniform} is given in Appendix.
...
Further work could include improving the speed, generating other types of distribution, and determining if the current algorithms are optimal or if further efficiency improvements are possible for a given store size.
