...
The basis of generating uniform variables (and other distributions) is to obtain a uniform variable $\sim \unif{rn}$ where $n$ is the number you need to generate, and $r$ is an integer. Extracting $\unif{rn}$ from an entropy store $S \sim \unif{s}$ is much easier than it sounds, because you just call $\textsc{sample}(S, s, s - s \mod n)$, which most of the time will return a uniform variable $\unif{rn}$ (and the other time, $\unif{s \mod n}$).
...
\Procedure{generate\_bernoulli}{$S, s, m, n$} 
  \State $S, s, r \gets \textsc{generate\_multiple}(S, s, n)$
  \State $S, s, B \gets \textsc{sample}(U_s, s, r * m)$
  \State \Return $S, s, B$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{proof}
    \begin{align}
    \mathbb{E}(\entropy{X,Y}) &=  \mathbb{E}(\entropy{X}) + \mathbb{E}(\entropy{Y}) \\
               ...
               & = \mathbb{E}(\entropy{Z})
    \end{align}
    Therefore the expected entropy of the input and outputs of $f_{weighted}$ is the same, and therefore $f_{weighted}$ does not lose entropy on average.
\end{proof}
