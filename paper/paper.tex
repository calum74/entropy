...
We'll use uppercase letters $X$, $Y$ and $Z$ for discrete random variables, and write $\entropy{X}$ for the Shannon entropy of this variable. Write $X \sim \unif{0..n-1}$ to mean that $X$ has a uniform integer distribution of "size" $n$, or $X \sim \unif{n}$ for short, because all uniform distributions will be 0-based. $\entropy{\unif{n}} = \log_2n$. The interval $[a,b)$ means all integers in the range $a...b-1$. Write $\mathbb{P}$ for probability, $\mathbb{E}$ for expected value, and lowercase $f$ as a function on random variables. $f^{-1}$ means the inverse function. $A \indep B$ means that random variables $A$ and $B$ are independent. In algorithms, $\div$ and $\mod$ are integer division and modulus operations, and subscripts are implemented as array access.
...
The function $f_{combine}$, in Definition \ref{def:combine}, allows us two combine two uniform variables $X$ and $Y$ into a single uniform variable $Z$. Its inverse $f^{-1}_{combine}$ in Lemma \ref{lem:divide}. $f_{combine}$ and $f^{-1}_{combine}$ divide the interval of size $nm$ into $n$ intervals of size $m$ as follows:
...
\begin{proof}
    Let $Z = mX+Y$, so $Z \in [0,nm)$. $\prob{X=x,Y=y} = \prob{X=x}\mathbb{P}(Y=y) = \frac{1}{n}\frac{1}{m} = \frac{1}{nm}$. Since $f_{combine}$ is a bijection then it means that all $z$ values occur with $\mathbb{P}(Z=z) = \frac{1}{nm}$, so $Z \sim Uniform\{nm\}$.
...
Algorithm \ref{alg:combine-uniform} is an implementation of $f_{combine}$, and Algorithm \ref{alg:divide-uniform} is an implementation of $f^{-1}_{combine}$. Algorithm \ref{alg:combine-uniform} will be used when \em adding \em entropy to the entropy store, and Algorithm \ref{alg:divide-uniform} can be used to \em extract \em entropy from the entropy store.
...
\Procedure{combine\_uniform}{$N, n, M, m$} 
  \State $S \gets N * m + M$
  \State $s \gets n * m$
  \State \Return $S, s$
\EndProcedure
...
\begin{algorithmic}[1]
    ...
    \Ensure $N \indep M$
\Procedure{divide\_uniform}{$S, s, m$} 
...
\begin{algorithmic}[1]
    ...
    \Ensure $X \indep B$
\Procedure{sample\_uniform}{$N, n, m$} 
  \If{$N < m$}
    \State $B \gets 1$  
    \State $x \gets m$
    \State $X \gets N$
