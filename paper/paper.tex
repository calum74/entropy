...
We'll use uppercase letters $X$, $Y$ and $Z$ for discrete random variables, and write $\entropy{X}$ for the Shannon entropy of this variable. Write $X \sim \unif{0..n-1}$ to mean that $X$ has a discrete uniform variable distribution of size $n$, or $X \sim \unif{n}$ for short. $\entropy{\unif{n}} = \log_2n$. The interval $[a,b)$ means all integers in the range $a...b-1$. Write $\mathbb{P}$ for probability, $\mathbb{E}$ for expected value, and lowercase $f$ as a function on random variables. $f^{-1}$ means the inverse function. $A \indep B$ means that random variables $A$ and $B$ are independent. In algorithms, $\div$ and $\mod$ are integer division and modulus operations, and subscripts $[]$ are array access.
...
\begin{definition}
    Let $\epsilon = \epsilon(p)$ be the expected entropy loss function of Algorithm \ref{alg:generate-multiple}, where $p=\frac{n-1}{s_{min}}$.
\end{definition}
...
\begin{corollary}
    The entropy loss of Algorithm \ref{alg:generate-uniform} is $\epsilon$.
\end{corollary}
...
We would expect this process to lose entropy because in general the output distribution contains less entropy than the input distribution. The new step is to notice that by splitting the interval $[0,n)$ into $k$ sub-intervals, we can also generate a uniform variable of size $w_i$ as follows:
...
\overbrace{
    ...
    }_{X \sim \textsc{Weighted}\{w_0, ..., w_{k-1}\}}
}^{Z \sim \unif{n}}
\]
...
\begin{definition}
    \label{def:o}
    For $k$ integer weights $w_i \ge 0$, let the offset $o_0$ = 0, $o_{i+1} = o_i + w_i$.
\end{definition}

\begin{definition}
    \label{def:p}
    For $k$ integer weights $w_i \ge 0$, let the output $p_j = i$ for $o_i \le j < o_{i+1}.$
\end{definition}

\begin{definition}
    Let $f_{weighted}: Z \rightarrow X, Y$ be the function that maps random variable $Z \sim \unif{n}$ to independent random variables $X \sim \text{Weighted}\{w\}$ and $Y \sim \unif{w_i}$.
...
\begin{algorithmic}[1]
\Require $S, s$ and $n$ are integers
\Require An array $w$ of $k$ integer weights $w_i \ge 0$
\Require An array $o$ of $k$ integer offsets  (see Definition \ref{def:o})
\Require An array $p$ integer outputs (see Definition \ref{def:p})
\Require $S \sim \unif{s}$
\Ensure $S \sim \unif{s}$
\Ensure $W \sim \textsc{Weighted}\{w\}$
\Ensure $S \indep W$
\Procedure{generate\_weighted}{$S, s, n, w, p, o$} 
    \State $S, s, r \gets \textsc{generate\_multiple}(S, s, n)$
    \State $W \gets p_{[S \div r]}$
    \State $s \gets r * w_{[W]}$
