...
\begin{abstract}
    ...
    The method introduces an \em entropy store \em that is available to cache unused entropy between invocations, and allows algorithms to be more efficient.
    The method can generate perfect random variates for any weighted integer distribution,
    and converges to optimal efficiency much more quickly than traditional batching techniques.
...
In this paper we will explore a fundamentally different approach, by allowing entropy conversion algorithms access to an \em entropy store \em (ES) in the form of a large uniformly distributed integer variable, and allow the algorithm to put any left over entropy back into the store. By first looking at the available operations on a uniform variable, we can compose them in such a way as to minimise the entropy loss.

The entropy efficiency depends only on the ratio of the size of the store to the size of the number generated, and an exact calculation is given in Theorem \ref{thm:loss}. For $n$ bits of stored entropy, the entropy loss is $\Theta(n2^{-n})$ and the efficiency is $1-\Theta(n2^{-n})$, which exceeds prior art which has efficiency $1-\Theta(n^{-1})$ \cite{kozen2022}.
...
A common proposal to reach asymptotic efficiency for random number generation is to use batching to generate multiple outputs at the same time. \cite{bacher2017,devroye86,han97,Knuth1976TheCO,kozen2022,lumbroso2013optimal} Batching spreads the entropy loss over the batch size, but does not recycle entropy beyond the batch, so is still constrained by the Knuth-Yao limit. However batching schemes suffer from increased size and complexity, and are less flexible because you must plan beforehand which numbers to generate. Batching does not always scale well because it requires arbitrary floating point precision or data structure sizes, particularly for the less optimised algorithms.

Kozen and Soloviev \cite{kozen2022} show that the efficiency of batching $k$ outputs is $1 - \Theta(k^{-1})$ in the general case, as well as developing an algebra and methods for combining randomness conversions, leveraging the efficiency gains of batching.
...
\begin{align}
    \eta & = \frac{\Entropy_{out}}{\Entropy_{in}}
          = \frac{\Entropy_{out}}{\Entropy_{out}+\epsilon} 
          \ge \frac{\log x}{\log x + \epsilon_{max}}
\end{align}
\end{proof}
...
\eta_{min} 
    & = \frac{\Entropy_{out}}{\Entropy_{out} + \epsilon} \\
    & = 1 - \frac{\epsilon}{\Entropy_{out} + \epsilon} \\
...
We have shown that the entropy efficiency limits of classical algorithms like Knuth-Yao \cite{Knuth1976TheCO} and the Interval Algorithm \cite{han97} can be overcome by using an entropy store, and that the efficiency converges to 1 faster than with traditional batching. 
