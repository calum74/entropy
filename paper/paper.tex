...
We'll use uppercase letters $X$, $Y$ and $Z$ for discrete random variables, and write $\entropy{X}$ for the Shannon entropy of this variable. Write $X \sim \unif{0..n-1}$ to mean that $X$ has a discrete uniform variable distribution of size $n$, or $X \sim \unif{n}$ for short. $\entropy{\unif{n}} = \log_2n$. $\bern{p}$ is a Bernoulli distribution.  The interval $[a,b)$ means all integers in the range $a...b-1$. Write $\mathbb{P}$ for probability, $\mathbb{E}$ for expected value, and lowercase $f$ as a function on random variables. $f^{-1}$ means the inverse function. $A \indep B$ means that random variables $A$ and $B$ are independent. In algorithms, $\div$ and $\mod$ are integer division and modulus operations, and subscripts $[]$ are array access.
...
\begin{corollary}
    Algorithms \ref{alg:combine}, \ref{alg:divide} and \ref{alg:sample} preserve entropy.
\end{corollary}
...
\Procedure{generate\_multiple}{$S, s, n$} 
  \While {True}
    ...
    \State $B, S, s \gets \textsc{sample}(S, s, s-r)$ 
    \If{$B$}
        \State \Return $S, s, k$
    \EndIf
  \EndWhile
...
\begin{proof}
$\epsilon(\frac{n-1}{s_{min}}) \rightarrow 0$ as $s_{min} \rightarrow \infty$, using L'H\^opital's Rule on Theorem \ref{thm:loss}. Therefore $\eta \rightarrow 1$ as $s_{min} \rightarrow \infty$.
\end{proof}
...
\Procedure{generate\_bernoulli}{$S, s, m, n$} 
  \State $S, s, r \gets \textsc{generate\_multiple}(U_s, s, n)$
  \State $B, S, s \gets \textsc{sample}(U_s, s, r * m)$
  \State \Return $S, s, B$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{lemma}
Proof of efficiency of Algorithm \ref{alg:generate-bernoulli}.
\end{lemma}
...
We would expect this process to lose entropy because in general the output distribution contains less entropy than the input distribution. The novel step is to notice that by splitting the interval $[0,n)$ into $k$ sub-intervals, we can also generate a uniform variable of size $w_i$ as follows:
...
Algorithm \ref{alg:generate-weighted} 

TODO: The variable k is confusing here. Maybe use $r$.
...
XX Check all the algorithms again, make sure they use the right letters XX

\begin{lemma}
    Proof of correctness of $f_{weighted}$.
\end{lemma}
...
Overall, ES is most suited when generating truly random integers from a hardware entropy source. When a pseudo-random source is used, then Lemire's algorithm is the fastest. Using a 64-bit ES was detrimental because the consumed entropy does not significantly change, but the operations are slower.
