...
No other algorithm has achieved this level of entropy efficiency in practise.
...
\begin{proof}
    $\prob{X=x} = \prob{X = t_z} = \frac{o_{x+1} - o_x}{n} = \frac{w_x}{n}$. Therefore $X \sim \mathrm{Weighted}\{w_0 ...\}$.  $\prob{Y=y} = \prob{Y = Z - o_X | X = x} = \prob{Y = Z - o_x | o_{x} \le Z < o_{x+1}} = \frac{1}{o_{x+1}-o_x} = \frac{1}{w_x}$. Therefore $Y \sim \unif{w_i}$ and $X \indep Y$.
\end{proof}
